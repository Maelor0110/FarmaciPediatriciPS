        ...
        {activeTab === 'farmaci' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              ...
              <div className="md:col-span-4 bg-white rounded-3xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between hover:border-slate-300 transition-all">
                <div className="flex items-start justify-between">
                  <div>
                    <span className="text-[11px] font-extrabold uppercase tracking-wider text-slate-400">Paziente Attivo</span>
                    <div className="flex items-baseline space-x-1 mt-1">
                      <span className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight font-mono">{patientWeight}</span>
                      <span className="text-base font-extrabold text-slate-500">kg</span>
                    </div>
                  </div>
                  {currentBroselow ? (
                    <div
                      className="px-3 py-1.5 rounded-2xl text-xs font-black border flex items-center space-x-1.5 shadow-xs"
                      style={{
                    ...
                    >
                      <span className="w-2.5 h-2.5 rounded-full ring-2 ring-white shadow-xs" style={{ backgroundColor: currentBroselow.colorHex }} />
              ...
              <div 
                ...
                className={`md:col-span-4 rounded-3xl p-5 border transition-all cursor-pointer flex flex-col justify-between shadow-xs ${
                  emergencyOnly
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <span className="text-[11px] font-extrabold uppercase tracking-wider text-rose-600 flex items-center space-x-1.5">
                      <span className="w-2 h-2 rounded-full bg-rose-500 animate-pulse" />
                      <span>Filtro Emergenza</span>
                    </span>
                    <h4 className="text-lg font-extrabold text-slate-900 tracking-tight">Farmaci Salvavita</h4>
                    <p className="text-xs text-slate-500">Adrenalina, Midazolam, Glucosio 10%, Morfina...</p>
                  </div>
                  <div className={`p-3 rounded-2xl border ${emergencyOnly ? 'bg-rose-600 text-white border-rose-600 shadow-md shadow-rose-600/30' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                    <Flame className="w-5 h-5" />
                  </div>
                </div>

                <div className="pt-4 mt-2 border-t border-slate-100/80 flex items-center justify-between text-xs">
                  ...
                  <span className={`text-[11px] font-bold px-2 py-0.5 rounded-lg border ${emergencyOnly ? 'bg-rose-600 text-white border-rose-600' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
              ...
              <div className="md:col-span-4 bg-white rounded-3xl p-5 border border-slate-200/90 shadow-xs flex flex-col justify-between hover:border-slate-300 transition-all">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h4 className="text-lg font-extrabold text-slate-900 tracking-tight mt-1">Calcolatore Dosi</h4>
                    <p className="text-xs text-slate-500">Dose mg/kg, limiti massimi e volumi siringa</p>
                  </div>
                  <button
                    onClick={() => setActiveTab('calcolatore')}
                    className="p-3 rounded-2xl bg-blue-50 border border-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-xs"
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200/90 shadow-xs space-y-4">
              <div className="flex flex-col md:flex-row gap-3">
                ...
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-10 py-3 bg-slate-50/80 border border-slate-200/90 rounded-2xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all placeholder:text-slate-400"
                  />
                  {searchQuery && (
                    <button
                      onClick={() => setSearchQuery('')}
                      className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 p-1.5 rounded-xl hover:bg-slate-200/60 transition-all"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  )}
                ...
                <div className="flex items-center space-x-2 shrink-0">
                  ...
                  {(searchQuery || selectedCategory !== 'all' || emergencyOnly || showOnlyFavorites) && (
                    <button
                      ...
                      className="p-2.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-2xl border border-slate-200/60 transition-all"
              ...
              <div className="flex items-center space-x-2 overflow-x-auto pb-1 scrollbar-none text-xs">
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl font-extrabold shrink-0 transition-all border ${
                ...
                {DRUG_CATEGORIES.map(cat => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3.5 py-2 rounded-xl font-bold shrink-0 transition-all flex items-center space-x-1.5 border ${
