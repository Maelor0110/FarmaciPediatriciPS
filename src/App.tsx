        ...
        {activeTab === 'farmaci' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 lg:gap-6">
              ...
              <div className="md:col-span-4 bg-white rounded-3xl p-5 md:p-6 lg:p-7 border border-slate-200/90 shadow-xs flex flex-col justify-between hover:border-slate-300 transition-all">
                <div className="flex items-start justify-between">
                  <div>
                    <span className="text-xs md:text-sm font-extrabold uppercase tracking-wider text-slate-400">Paziente Attivo</span>
                    <div className="flex items-baseline space-x-1.5 mt-1 md:mt-2">
                      <span className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 tracking-tight font-mono">{patientWeight}</span>
                      <span className="text-base md:text-xl font-extrabold text-slate-500">kg</span>
                    </div>
                  </div>
                  {currentBroselow ? (
                    <div
                      className="px-3 md:px-4 py-1.5 md:py-2 rounded-2xl text-xs md:text-sm font-black border flex items-center space-x-2 shadow-xs"
                      style={{
                    ...
                    >
                      <span className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full ring-2 ring-white shadow-xs" style={{ backgroundColor: currentBroselow.colorHex }} />
                      <span>{currentBroselow.color}</span>
                    </div>
                  ) : (
                    <span className="text-xs md:text-sm font-bold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-xl">
                      {patientWeight > 36 ? 'Adolescente' : 'Neonato'}
                    </span>
                  )}
                </div>

                <div className="pt-4 md:pt-5 mt-2 border-t border-slate-100 flex items-center justify-between text-xs md:text-sm">
              ...
              <div 
                ...
                className={`md:col-span-4 rounded-3xl p-5 md:p-6 lg:p-7 border transition-all cursor-pointer flex flex-col justify-between shadow-xs ${
                  emergencyOnly
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="space-y-1 md:space-y-1.5">
                    <span className="text-xs md:text-sm font-extrabold uppercase tracking-wider text-rose-600 flex items-center space-x-1.5">
                      <span className="w-2 h-2 md:w-2.5 md:h-2.5 rounded-full bg-rose-500 animate-pulse" />
                      <span>Filtro Emergenza</span>
                    </span>
                    <h4 className="text-lg md:text-xl lg:text-2xl font-black text-slate-900 tracking-tight">Farmaci Salvavita</h4>
                    <p className="text-xs md:text-sm text-slate-500">Adrenalina, Midazolam, Glucosio 10%, Morfina...</p>
                  </div>
                  <div className={`p-3 md:p-4 rounded-2xl border ${emergencyOnly ? 'bg-rose-600 text-white border-rose-600 shadow-md shadow-rose-600/30' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                    <Flame className="w-5 h-5 md:w-6 md:h-6" />
                  </div>
                </div>

                <div className="pt-4 md:pt-5 mt-2 border-t border-slate-100/80 flex items-center justify-between text-xs md:text-sm">
                  <span className={`font-bold ${emergencyOnly ? 'text-rose-700' : 'text-slate-600'}`}>
                    {emergencyOnly ? 'Filtro ATTIVO' : `${emergencyCount} farmaci prioritari`}
                  </span>
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-lg border ${emergencyOnly ? 'bg-rose-600 text-white border-rose-600' : 'bg-slate-100 text-slate-700 border-slate-200'}`}>
              ...
              <div className="md:col-span-4 bg-white rounded-3xl p-5 md:p-6 lg:p-7 border border-slate-200/90 shadow-xs flex flex-col justify-between hover:border-slate-300 transition-all">
                <div className="flex items-start justify-between">
                  <div className="space-y-1 md:space-y-1.5">
                    <span className="text-xs md:text-sm font-extrabold uppercase tracking-wider text-blue-600 flex items-center space-x-1.5">
                      <Calculator className="w-4 h-4" />
                      <span>Calcolo Interattivo</span>
                    </span>
                    <h4 className="text-lg md:text-xl lg:text-2xl font-black text-slate-900 tracking-tight">Calcolatore Dosi</h4>
                    <p className="text-xs md:text-sm text-slate-500">Dose mg/kg, limiti massimi e volumi siringa</p>
                  </div>
                  <button
                    onClick={() => setActiveTab('calcolatore')}
                    className="p-3 md:p-4 rounded-2xl bg-blue-50 border border-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-xs"
            ...
            <div className="bg-white rounded-3xl p-5 md:p-7 lg:p-8 border border-slate-200/90 shadow-xs space-y-4 md:space-y-5">
              <div className="flex flex-col md:flex-row gap-3 md:gap-4">
                {/* Search Bar */}
                <div className="relative flex-1">
                  <Search className="w-5 h-5 md:w-6 md:h-6 text-slate-400 absolute left-4 md:left-5 top-1/2 -translate-y-1/2" />
                  <input
                    type="text"
                    placeholder="Cerca farmaco, nome commerciale (es. Tachipirina, Keppra, Bentelan) o indicazione..."
                    value={searchQuery}
                    onChange={e => setSearchQuery(e.target.value)}
                    className="w-full pl-11 md:pl-14 pr-10 md:pr-12 py-3 md:py-4 lg:py-4.5 bg-slate-50/80 border border-slate-200/90 rounded-2xl text-sm md:text-base lg:text-lg font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all placeholder:text-slate-400"
                  />
                  {searchQuery && (
                    <button
                      onClick={() => setSearchQuery('')}
                      className="absolute right-3.5 md:right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 p-1.5 md:p-2 rounded-xl hover:bg-slate-200/60 transition-all"
                    >
                ...
                <div className="flex items-center space-x-2 md:space-x-3 shrink-0">
                  <button
                    onClick={() => setEmergencyOnly(!emergencyOnly)}
                    className={`px-3.5 md:px-5 py-2.5 md:py-3.5 rounded-2xl text-xs md:text-sm lg:text-base font-black flex items-center space-x-2 transition-all border ${
                      emergencyOnly
                  ...
                  >
                    <Bookmark className={`w-4 h-4 md:w-5 md:h-5 ${showOnlyFavorites ? 'fill-white' : ''}`} />
                    <span>Solo Preferiti ({favorites.length})</span>
                  </button>
                  ...
                  {(searchQuery || selectedCategory !== 'all' || emergencyOnly || showOnlyFavorites) && (
                    <button
                      ...
                      className="p-2.5 md:p-3.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-2xl border border-slate-200/60 transition-all"
              ...
              <div className="flex items-center space-x-2 md:space-x-2.5 overflow-x-auto pb-1 scrollbar-none text-xs md:text-sm">
                <button
                  onClick={() => setSelectedCategory('all')}
                  className={`px-3.5 md:px-5 py-2 md:py-2.5 rounded-xl md:rounded-2xl font-black shrink-0 transition-all border ${
                ...
                {DRUG_CATEGORIES.map(cat => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3.5 md:px-4 py-2 md:py-2.5 rounded-xl md:rounded-2xl font-bold shrink-0 transition-all flex items-center space-x-1.5 md:space-x-2 border ${
                    ...
                    >
                      <span>{cat.name}</span>
                      <span className="text-[10px] md:text-xs opacity-80 font-bold">({count})</span>
                    </button>
                  );
                })}
              </div>
